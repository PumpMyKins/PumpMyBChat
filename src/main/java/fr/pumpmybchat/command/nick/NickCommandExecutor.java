...
public class NickCommandExecutor extends Command implements TabExecutor{
	
	private Main main;
	private List<SubCommandData> subCommandList;
	
	private NickCommandExecutor(String name) {
		super(name);
		this.subCommandList = new ArrayList<>();
	}

	public NickCommandExecutor(Main main) {
	...
	public void execute(CommandSender sender, String[] args) {
		...
		if( args.length < 1 ) {
			...
			new HelpNickSubCommand().onSubCommand(this, sender);
			return;
