...
public class HelpNickAdminSubCommand implements ISubCommand{
	...
	public void onSubCommand(Command exec, CommandSender sender, List<String> args) {
		...
		TextComponent message = new TextComponent("----[HELP]Nick-Admin[HELP]----");
		...
		TextComponent block = new TextComponent("/nick-admin block");
		block.setColor(ChatColor.DARK_BLUE);
		block.setBold(true);
		block.setClickEvent(new ClickEvent(ClickEvent.Action.SUGGEST_COMMAND, "/nick-admin block"));
		block.setHoverEvent(new HoverEvent(Action.SHOW_TEXT,new ComponentBuilder("§bExecuter la commande !").create()));
		TextComponent txt = new TextComponent(" pour bloquer ou débloquer le surnom d'un joueur.");
