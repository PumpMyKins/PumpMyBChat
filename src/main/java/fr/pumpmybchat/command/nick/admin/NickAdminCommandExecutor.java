...
public class NickAdminCommandExecutor extends Command implements TabExecutor{
	...
	private NickAdminCommandExecutor(String name) {
		super(name);
		this.subCommandList = new ArrayList<>();
	}

	public NickAdminCommandExecutor(Main main) {
	...
	public void execute(CommandSender sender, String[] args) {
		...
		if(!sender.hasPermission("pumpmybchat.command.nick.admin")) {
		...
		if( args.length < 1 ) {
			...
			new HelpNickAdminSubCommand().onSubCommand(this, sender);
			return;
