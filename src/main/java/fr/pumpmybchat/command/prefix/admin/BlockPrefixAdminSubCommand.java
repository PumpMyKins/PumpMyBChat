...
public class BlockPrefixAdminSubCommand implements ISubCommand, ISubTabCompleter {

	private Main main;

	public BlockPrefixAdminSubCommand(Main main) {
		this.main = main;
	}
	...
	public void onSubCommand(Command exec, CommandSender sender, List<String> args) {
		...
		if(args.size() == 1 && Lists.newArrayList(this.onTabComplete(exec,sender, args)).contains(args.get(0))) {
			...
			if(prefix.isBlocked()) {
				blocked = false;
				sender.sendMessage(new TextComponent(Main.PLUGIN_PREFIX + "§bPrefix débloqué pour le joueur " + player.getName() + "!"));
			}else {
				blocked = true;
				sender.sendMessage(new TextComponent(Main.PLUGIN_PREFIX + "§bPrefix bloqué pour le joueur " + player.getName() + " !"));			
			}
